...
interface EditDriverModalProps {
  ...
  export default function EditDriverModal({ isOpen, onClose, onSuccess, driverId }: EditDriverModalProps) {
    ...
    // Reset fetch state when driverId changes
    useEffect(() => {
      setHasFetched(false);
    }, [driverId]);
    ...
    useEffect(() => {
      ...
      const fetchDriverData = async () => {
        if (!isOpen || !driverId || hasFetched || isFetchingDriver) return;
        ...
        try {
          ...
          if (driver.contracts && driver.contracts.length > 0) {
            setContractsData(driver.contracts.map(contract => ({
              file: contract.file || '',
    ...
    }, [isOpen, driverId, hasFetched, isFetchingDriver]); // Removed getDriver to prevent loops
